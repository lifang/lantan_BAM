...
class ViolationReward < ActiveRecord::Base
  ...
  def self.vio_reward()
    return ViolationReward.joins(:staff).select("sum(score_num) score,staff_id,types").where("date_format(v.process_at,'%Y-%m')=date_format(DATE_SUB(curdate(), INTERVAL 1 MONTH),'%Y-%m')
    and staff.status !=#{Staff::STATUS[:deleted]} and violation_rewards.status=#{ViolationReward::STATUS[:PROCESSED]}").group("types,staff_id").inject(Hash.new){|hash,v|
