...
class PackageCard < ActiveRecord::Base
  ...
  #查询卡信息
  def self.search_pcard(store_id,page,pcard=nil,car_num=nil,c_name=nil,created_at=nil,ended_at=nil)
    sql="select c.name,c.mobilephone,cp.content,n.num,p.price,cp.status from c_pcard_relations cp inner join customers c on c.id=cp.customer_id
    ...
    on n.id=cn.car_num_id where p.store_id=#{store_id}"
    ...
    return CPcardRelation.paginate_by_sql(sql, :page => page, :per_page => 5)
  end
end
