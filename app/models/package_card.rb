...
class PackageCard < ActiveRecord::Base
  ...
  def self.search_pcard(store_id,pcard=nil,car_num=nil,c_name=nil,created_at=nil,ended_at=nil)
    sql="select cp.id,c.name,c.mobilephone,p.name p_name,cp.content,p.price,cp.status from c_pcard_relations cp inner join customers c on c.id=cp.customer_id
    inner join  package_cards p on p.id=cp.package_card_id inner join customer_num_relations  cn on c.id=cn.customer_id 
