...
class Sale < ActiveRecord::Base
  ...
  def self.count_sale_orders(store_id)
    sql ="select count(o.id) o_num,sum(o.price) sum,concat_ws('--',date_format(s.started_at,'%Y.%m.%d'),date_format(s.ended_at,'%Y.%m.%d')) day,
         s.introduction intro,sum(p.price) sale_price,s.name from sales s  inner join orders o on s.id=o.sale_id left join order_pay_types p on
         p.order_id=o.id where s.store_id=? and p.pay_type=? group by s.id;"
    return Sale.find_by_sql([sql,store_id,OrderPayType::PAY_TYPES[:SALE]])
  end
  ...
  def self.count_sale_orders_search(store_id,started_at=nil,ended_at=nil,name=nil)
    sql ="select count(o.id) o_num,sum(o.price) sum,concat_ws('--',date_format(s.started_at,'%Y.%m.%d'),date_format(s.ended_at,'%Y.%m.%d')) day,
         s.introduction intro,sum(p.price) sale_price,s.name from sales s  inner join orders o on s.id=o.sale_id left join order_pay_types p on
         p.order_id=o.id where s.store_id=#{store_id} and p.pay_type=#{OrderPayType::PAY_TYPES[:SALE]}"
