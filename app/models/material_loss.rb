...
#  end
  def self.loss_list store_id,types=nil,name=nil,code=nil
    sql = ["select ml.id, ml.loss_num,ml.store_id, m.code, m.name, m.status, m.types, m.unit, m.price, m.sale_price,
    s.name staff_name, c.name cname from material_losses ml inner join materials m on ml.material_id=m.id inner join categories c
