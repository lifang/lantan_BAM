...
class CarNum < ActiveRecord::Base
  ...
  def self.import_d(store_id)
    ...
    c_store_ralation,c_num_relation,prod_mat_relation,order_prod_relation,order_pay_type = [],[],[],[],[]
    Spreadsheet.open path+"rpprocesshead.xls" do |book|  #客户表  使用block可以关闭文件
      ...
      sheet.each_with_index do |row,index|
        if index !=0 and row[58]
          ...
          customers["#{row[114]}-#{row[98]}"] = {:name=>row[22],:mobilephone=>row[83].nil? ? row[119] : row[83]}
          c_orders["#{row[114]}-#{row[98]}"].nil? ? c_orders["#{row[114]}-#{row[98]}"]=[row[46].strip=>row[30]] : c_orders["#{row[114]}-#{row[98]}"] << {row[46].strip=>row[30]}
        end
      end
    end
    repaire_book = Spreadsheet.open path+"rpplus.xls" do |repaire_book| #维修工单
      ...
      repaire_sheet.each_with_index do |row,index|
        name = row[16].strip.gsub(/(\(|（)[^（\(\)）]*?(\)|）)、\*\/\D.\d./, "").strip
        if name != "" && name.length >0
          ...
          pro_num[row[12].strip].nil? ? pro_num[row[12].strip] = [row[23]] : pro_num[row[12].strip] << row[23]
        end
      end
    end
    p order_ids.values.flatten.length
    #    begin
    Customer.transaction do
      ...
      suit_orders = []
      n_orders.values.flatten.each {|o|o.each{|k,v|suit_orders << k}}
      ...
      code = store_id.to_s + Time.now.strftime("%Y%m%d%H%M%S")
      n = 0
      p order_prod.keys.length
      n_orders.each do |k,v|
        v.each do |h|
          h.each_pair{|key,value|
            n += 1
            order_parm = {:car_num_id=>k.split("-")[1],:customer_id=>k.split("-")[0],:price=>value,:status=>Order::STATUS[:BEEN_PAYMENT],
              :store_id=>store_id,:code=>code+add_length(5,n)}
            order = Order.create(order_parm)
            order_pay_type << OrderPayType.new(:order_id=>order.id,:pay_type=>0,:price=>value)
            num = pro_num[key].nil? ? 1 : pro_num[key]
            order_prod[key].each do |prod|
              order_prod_relation << OrderProdRelation.new(:order_id=>order.id,:product_id=>prod,:price=>value,:total_price=>value,:pro_num=>num)
