...
class Complaint < ActiveRecord::Base
  ...
  def self.one_customer_complaint(store_id, customer_id, per_page, page)
   return Complaint.paginate_by_sql(["select c.created_at, c.reason, c.suggstion, c.types, c.status, c.remark,
          st.name st_name1, st2.name st_name2, o.code from complaints c left join orders o on o.id = c.order_id
