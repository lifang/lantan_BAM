...
class OrderPayType < ActiveRecord::Base
  ...
  def self.deal_order(param)
    may_pay,card_price,msg,orders,is_billing = true,{},"付款成功！",[],param[:pay_order][:is_billing].to_i
    ...
    if may_pay   #如果密码正确
      OrderPayType.transaction do
        ...
        unless orders.blank?
          ...
          revist = prods.group_by{|i|i.o_id}.each{|k,v|revist[k] = v.map(&:revist_content).compact unless send_orders[k].auto_time.nil?}
          ...
          if may_pay
            ...
            if param[:pay_order] && param[:pay_order][:loss_ids]
              ...
              loss.each do |k,v|
                OrderPayType.create(:order_id=>k,:price=>v.to_f.round(2),:pay_type=>OrderPayType::PAY_TYPES[:FAVOUR])
            ...
            csvs.each do |csr|   #如果是新买储值卡则生成购买记录
            ...
            if c_customer && c_customer.is_vip
             ...
             order_prod_relations.order_id o_id").where(:"orders.id"=>order_ids).inject({}){|hash,p|hash[p.o_id] = p.point if p.point;hash}
              ...
              #生成积分记录
              t_point = order_point.merge(pcard_point).inject([]){|arr,p| arr << Point.new(:customer_id=>param[:customer_id],
