...
class WorkOrdersController < ApplicationController
  def work_orders_status
      ...
      stations.each do |station| #遍历所有工位
        if station.status == Station::STAT[:NORMAL]
          ...
          if work_orders != nil
            ...
            station[:s_name] = nil unless station[:s_name]
    ...
    head_sales = hs.inject([]){|h, s|
      h << Constant::HEAD_OFFICE_API_PATH + s.img_url unless s.img_url.nil? || s.img_url.strip == "";
      h
    }
    ...
    local_sales = ls.inject([]){|h, s|
      h << Constant::SERVER_PATH + s.img_url unless s.img_url.nil? || s.img_url.strip == "";
      h
    }
