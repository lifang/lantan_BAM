...
class SalariesController < ApplicationController
  ...
  def update
    ...
    if salary.is_edited
      ...
      pre_total = salary.reward_num - salary.voilate_fee + salary.work_fee + salary.manage_fee - salary.tax_fee +  reward_fee + salary.deduct_num
      total_price = params[:reward_num].to_f.round(2) - params[:voilate_fee].to_f.round(2) + params[:work_fee].to_f.round(2) +
        params[:manage_fee].to_f.round(2) - params[:tax_fee].to_f.round(2)+ params[:deduct_num].to_f.round(2) + params[:reward_fee].to_f.round(2)
