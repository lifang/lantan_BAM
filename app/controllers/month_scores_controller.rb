...
class MonthScoresController < ApplicationController
  ...
  def create
    ...
    params[:goal].each do |k,v|
      ...
      parm_type.merge!(:type_name=>type_name,:goal_price=>v,:types=>types)  unless type_name==""
      GoalSaleType.create(parm_type)
      index +=1 if k.to_i >=4
    end
    redirect_to "/stores/#{params[:store_id]}/month_scores/makets_goal"
  end
  ...
  def search_sale_order
    ...
    redirect_to "/stores/#{params[:store_id]}/month_scores/sale_order_list"
  end
