...
module ApplicationHelper
  ...
  def new_app_order_by_status(orders)
    order_hash = {}
    order_hash[Order::STATUS[:WAIT_PAYMENT]] = orders.select{|order| order.status == Order::STATUS[:WAIT_PAYMENT] || order.status == Order::STATUS[:COMMIT] || order.status == Order::STATUS[:PCARD_PAY]}
